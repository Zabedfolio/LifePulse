<?php
// filepath: requests/add_request.php
error_reporting(E_ALL);
ini_set('display_errors', 1);

include __DIR__ . '/../db_connect.php';
include __DIR__ . '/request_functions.php'; // For createRequest
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_request'])) {
    $data = [
        ...
        'status' => 'pending' // Default for public requests
    ];
    ...
    if ($data['patient_name'] && $data['blood_group'] && $data['units_needed'] > 0 && $data['contact']) {
        ...
        if ($res['ok']) {
            ...
            $message = '❌ Error: ' . ($res['error'] ?? 'Failed to submit request');
        }
    } else {
